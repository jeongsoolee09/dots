...
values."
  (setq-default
   ...
   '(;; ============ Spacemacs functionalities ============
     ...
     (elfeed :variables rmh-elfeed-org-files (list "~/.emacs.d.spacemacs/elfeed.org"))
     (mu4e :variables mu4e-installation-path (if (string-match "aarch64.*" system-configuration)
                                                 "/opt/homebrew/Cellar/mu/1.8.10/share/emacs/site-lisp/mu/mu4e"
                                                 "/usr/local/Cellar/mu/1.8.10/share/emacs/site-lisp/mu/mu4e"))
     ...
     (javascript :variables javascript-backend nil
                 js2-basic-offset 2)
     (typescript :variables typescript-backend nil)
     (solidity :variables solidity-flycheck-solium-checker-active t)
     (clojure :variables clojure-backend nil
     ...
     (haskell :variables haskell-backend 'dante)
     (purescript :variables purescript-backend nil)
     (elm :variables elm-backend nil)
     idris
     ;; agda
     (ocaml :variables ocaml-backend nil)
     (markdown :variables markdown-live-preview-engine 'vmd)
     asciidoc
     restructuredtext
     (python :variables python-backend nil)
     (fsharp :variables fsharp-backend nil)
     ...
     mermaid
     (vimscript :variables vimscript-backend nil)
     (rust :variables rust-backend nil)
     (go :variables go-backend 'go-mode)
     (kotlin :variables kotlin-backend nil)
     (lua :variables lua-backend nil)
     (sql :variables sql-backend nil)
     (elixir :variables elixir-backend nil)
     (erlang :variables erlang-backend nil)
     tidalcycles)
   ...
   dotspacemacs-additional-packages '(transpose-frame emms sicp s ts multi-vterm eradio
                                                      ...
                                                      coterm plz twitch-api denote auto-dark)
   ;; A list of packages that cannot be updated.
   dotspacemacs-frozen-packages '()
   ;; A list of packages that will not be installed and loaded.
   dotspacemacs-excluded-packages '(meghanada anaconda-mode merlin
                                    lsp-mode lsp-docker lsp-treemacs helm-lsp
                                    lsp-haskell lsp-julia lsp-ui lsp-metals
                                    inspector)
...
you should place your code here."
  ...
  (add-to-list 'load-path "~/.emacs.d.spacemacs/lisp/")
  (add-to-list 'load-path "~/.emacs.d.spacemacs/private/local/custom-lisp/")
  ...
  (with-eval-after-load 'kotlin-mode
    ...
    (define-key kotlin-mode-map (kbd "TAB") (lambda () (interactive) (dotimes (_ 4) (insert " "))))
  ...
  ;; Keybinding FLEX
  ;; command-shortcuts
  ;; (define-key (current-global-map) (kbd "H-i")
  ...
  (global-set-key (kbd "C-H-c") 'spacemacs/toggle-global-rainbow-identifiers-mode)
  ...
  (spacemacs/set-leader-keys "H-k" 'helm-show-kill-ring)
  (spacemacs/set-leader-keys "H-x" 'delete-trailing-whitespace)
  (spacemacs/set-leader-keys "H-b" 'helm-bookmarks)
  ...
  ;; C-shortcuts
  (evil-define-key 'normal 'prog-mode-map (kbd "C-=") #'spacemacs/indent-region-or-buffer)
  (evil-define-key 'insert 'prog-mode-map (kbd "C-=") #'spacemacs/indent-region-or-buffer)
  (evil-define-key 'insert 'prog-mode-map (kbd "C-i") #'hippie-expand)
  ...
  (with-eval-after-load 'python-mode
    (setq python-shell-setup-codes '(python-shell-eval-setup-code
                                     python-shell-eval-file-setup-code
                                     python-shell-completion-setup-code))
    ;; TODO borken: command-execute: The mark is not set now, so there is no region
...
(custom-set-faces
 ...
 '(pdf-view-midnight-colors ((t (("#ffffff" . "#000000")))))
